...
document
  ...
  if (panel) {
    ...
    let transcriptRange = 'all';
    let transcriptPage = 1;
    let transcriptPagination = { page:1, pages:1, total:0 };

    async function loadStaffTickets(closedOnly = false) {
      const params = new URLSearchParams();
      if (closedOnly) { params.set('status','Closed'); params.set('search', transcriptSearch); params.set('range', transcriptRange); params.set('page', transcriptPage); params.set('page_size','12'); }
      else params.set('active','1');
      const result = await apiRequest(`/api/staff/tickets?${params.toString()}`);
      staffTickets = (result.tickets || []).map(normaliseTicket);
      if (closedOnly) transcriptPagination = result.pagination || {page:1,pages:1,total:staffTickets.length};
      return staffTickets;
    }
    ...
    async function renderTicketInbox(closedOnly=false) {
      ...
      try {
        const arr = await loadStaffTickets(closedOnly); stats();
        const toolbar = closedOnly ? `<div class="transcript-toolbar"><label><span>SEARCH TRANSCRIPTS</span><input id="transcript-search" value="${esc(transcriptSearch)}" placeholder="Reference, player, Union ID, Discord ID, subject..."></label><label><span>DATE RANGE</span><select id="transcript-range"><option value="all">All transcripts</option><option value="today" ${transcriptRange==='today'?'selected':''}>Today</option><option value="week" ${transcriptRange==='week'?'selected':''}>Last 7 days</option><option value="month" ${transcriptRange==='month'?'selected':''}>Last 30 days</option></select></label><button class="primary-button" id="transcript-search-button">Search</button></div>` : '';
        const pager = closedOnly && transcriptPagination.pages>1 ? `<div class="transcript-pager"><button ${transcriptPagination.page<=1?'disabled':''} data-transcript-page="${transcriptPagination.page-1}">Previous</button><span>Page ${transcriptPagination.page} of ${transcriptPagination.pages} · ${transcriptPagination.total} total</span><button ${transcriptPagination.page>=transcriptPagination.pages?'disabled':''} data-transcript-page="${transcriptPagination.page+1}">Next</button></div>` : '';
        panel.innerHTML = `${toolbar}<div class="staff-ticket-workspace ${closedOnly?'transcript-workspace':''}"><aside class="staff-ticket-inbox"><div class="staff-inbox-head"><span>${closedOnly?'TRANSCRIPTS':'LIVE TICKETS'}</span><strong>${closedOnly?transcriptPagination.total:arr.length}</strong></div>${ticketRows(arr, closedOnly?'No transcripts found':'No tickets', closedOnly?'Try a different search or date range.':'New tickets will appear here.')}${pager}</aside><section id="staff-live-thread" class="staff-live-thread"><div class="ticket-empty-state"><strong>Select a ${closedOnly?'transcript':'conversation'}</strong><p>Choose a ticket to view its full history.</p></div></section></div>`;
        const runSearch=()=>{transcriptSearch=panel.querySelector('#transcript-search')?.value.trim()||'';transcriptRange=panel.querySelector('#transcript-range')?.value||'all';transcriptPage=1;activeStaffTicket=null;renderTicketInbox(true)};
        panel.querySelector('#transcript-search-button')?.addEventListener('click',runSearch);
        panel.querySelector('#transcript-search')?.addEventListener('keydown',e=>{if(e.key==='Enter')runSearch()});
        panel.querySelector('#transcript-range')?.addEventListener('change',runSearch);
        panel.querySelectorAll('[data-transcript-page]').forEach(b=>b.addEventListener('click',()=>{transcriptPage=Number(b.dataset.transcriptPage);activeStaffTicket=null;renderTicketInbox(true)}));
        panel.querySelectorAll('[data-staff-ticket]').forEach(b=>b.addEventListener('click', async()=>{activeStaffTicket=b.dataset.staffTicket;await apiRequest(`/api/staff/tickets/${encodeURIComponent(activeStaffTicket)}/read`,{method:'POST',body:'{}'});await renderTicketInbox(closedOnly);await renderStaffThread(closedOnly);}));
    ...
    async function renderStaffThread(transcriptMode=false){
      ...
      const messages=parseTicketMessages(t), audit=Array.isArray(result.ticket.audit)?result.ticket.audit:[]; const banInfo=t.banId?` · Ban ID: ${esc(t.banId)}`:'';
      const remaining=remainingClosedTime(t), duration=Math.max(0,Number(t.closed_at||Date.now())-Number(t.created_at||Date.now()));
      const statusControl = transcriptMode ? `<div class="transcript-actions"><span>${remaining>0?`Deletes in ${durationText(remaining)}`:'Deletion due'}</span><button type="button" class="secondary-button" id="staff-download-transcript">Download HTML</button><button type="button" class="secondary-button" id="staff-copy-transcript">Copy</button>${remaining>0?'<button type="button" class="primary-button" id="staff-reopen-ticket">Reopen</button>':''}<button type="button" class="danger-button" id="staff-delete-transcript">Delete permanently</button></div>` : `<select id="staff-ticket-status">${['Open','In Progress','Awaiting Player','Closed'].map(s=>`<option ${t.status===s?'selected':''}>${s}</option>`).join('')}</select>`;
      const info = transcriptMode ? `<div class="transcript-summary"><article><span>PLAYER</span><strong>${esc(t.name)}</strong><small>${esc(t.discord_id||'No Discord ID')}</small></article><article><span>UNION ID</span><strong>${esc(t.union_id||'Not recorded')}</strong><small>${esc(t.category)}</small></article><article><span>CLOSED BY</span><strong>${esc(t.closed_by_name||'Unknown staff')}</strong><small>${fmt(t.closed_at)}</small></article><article><span>TICKET DURATION</span><strong>${durationText(duration)}</strong><small>${messages.length} messages</small></article></div>` : '';
      const auditHtml=transcriptMode?`<details class="transcript-audit"><summary>Audit history <b>${audit.length}</b></summary><div>${audit.length?audit.map(a=>`<article><i></i><div><strong>${esc(a.action)}</strong><p>${esc(a.old_value||'')} ${a.old_value||a.new_value?'→':''} ${esc(a.new_value||'')}</p></div><time>${fmt(a.created_at)}</time></article>`).join(''):'<p>No audit entries recorded for this ticket.</p>'}</div></details>`:'';
      const composer = transcriptMode ? `<div class="chat-closed-notice"><strong>Closed transcript</strong><p>This record is retained for 48 hours after closure unless reopened or permanently deleted by staff.</p></div>` : `<form class="ticket-chat-composer" id="staff-chat-form"><textarea id="staff-chat-text" rows="3" maxlength="2000" placeholder="Reply as Union Staff..."></textarea><div class="composer-actions"><label class="attachment-button">Attach images<input id="staff-chat-files" type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple></label><span id="staff-chat-file-name"></span><button class="primary-button" type="submit">Send Reply</button></div><p class="ticket-form-message" id="staff-chat-message"></p></form>`;
      holder.innerHTML=`<header class="thread-header"><div><span>${esc(t.reference)}</span><h2>${esc(t.subject)}</h2><p>${esc(t.name)} · ${esc(t.category)} · ${esc(t.priority)}${banInfo}</p></div>${statusControl}</header>${info}<div class="ticket-chat-log staff-log">${messages.map(m=>`<article class="chat-message ${m.sender==='staff'?'staff':m.sender==='system'?'system':'player'}"><div class="chat-message-meta"><strong>${esc(m.sender_name || m.senderName || (m.sender==='staff'?'Union Staff':t.name))}</strong><span>${fmt(m.created_at || m.createdAt)}</span></div>${m.text?`<p>${esc(m.text).replace(/\n/g,'<br>')}</p>`:''}${attachmentHtml(m.attachments || [])}</article>`).join('')}</div>${auditHtml}${composer}`;
      ...
      holder.querySelector('#staff-download-transcript')?.addEventListener('click',async()=>{const response=await fetch(`${API_BASE}/api/staff/tickets/${encodeURIComponent(t.reference)}/export`,{headers:{Authorization:`Bearer ${getAccessToken()}`}});if(!response.ok)throw new Error('Could not download transcript.');const blob=await response.blob(),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${t.reference}-transcript.html`;a.click();URL.revokeObjectURL(url)});
      holder.querySelector('#staff-copy-transcript')?.addEventListener('click',async()=>{const body=messages.map(m=>`[${fmt(m.created_at||m.createdAt)}] ${m.sender_name||m.senderName}: ${m.text||'[attachment]'}`).join('\n\n');await navigator.clipboard.writeText(`${t.reference} — ${t.subject}\nPlayer: ${t.name}\n\n${body}`);holder.querySelector('#staff-copy-transcript').textContent='Copied'});
      holder.querySelector('#staff-delete-transcript')?.addEventListener('click',async()=>{if(!confirm(`Permanently delete ${t.reference}? This cannot be undone.`))return;await apiRequest(`/api/staff/tickets/${encodeURIComponent(t.reference)}/delete`,{method:'DELETE'});activeStaffTicket=null;await renderTicketInbox(true)});
